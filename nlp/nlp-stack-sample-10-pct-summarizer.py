...
def load_dataset_as_dataframe(path, n_obs, features_include):
    dataframe = pd.read_csv(
        path, nrows=n_obs, usecols=features_include, encoding="latin1"
    )
...
def clean_text(text_series):
    ...
    cleaned_text_series = re.sub(
        "<code>.*?</code>", "", cleaned_text_series, flags=re.DOTALL
    )
...
def all_process_text(text_series):
    normalized_text_series = text_series.apply(clean_text)
    scrubbed_text_series = normalized_text_series.apply(
        lambda x: scrub_and_tokenize(x, False)
    )
    return normalized_text_series, scrubbed_text_series
...
def generate_summarize_text(text_without_removing_dot, cleaned_text):
    ...
    summary_sentences = sentence_scores[sent].nlargest(7, sentence_scores, key=sentence_scores.get)
...
def main():
    ...
    df_questions["Body_Cleaned_1"], df_questions["Body_Cleaned"] = all_process_text(
        df_questions["Body"]
    )
    print(df_questions["Body"][0])
    plot_distribution_of_scores(df_questions["Score"])
    generate_summarize_text(
        df_questions["Body_Cleaned_1"][10], df_questions["Body_Cleaned"][10]
    )
